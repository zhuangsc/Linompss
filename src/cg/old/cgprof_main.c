...
//		#pragma omp taskwait
//		svec_sanity_check("alpha1", alpha1[i], n);
	}
#else
	/* p[0] = r[0] */
	BBLAS_COPY(PRIOR_FIRSTCP, bm, bn, n, s, r[i], p[i]); 				
	...
	for ( k=0; k<steps; ++k ) {
		...
		if ( async && warmup<=0 && ncorrection==-1) {
			...
			#endif
			BBLAS_DOT_SCHED_ASYNC_CONCURRENT(k, &sync[DOT_ALPHA2], PRIOR_ASYNCDOT, bm, bn, n, s, tmp[i], p[iprev], alpha2[i], release, bm_p); // alpha2[i] = <tmp[i], p[i]> (partial)
			BBLAS_SCAL_CPAXPY_COMB_ASYNC_CONCURRENT(k, &sync[DOT_ALPHA2], PRIOR_ASYNCAXPY, bm, bn, n, s, -1.0, alpha1[iprev], alpha2[i], tmp[i], p[iprev], r[iprev], x[iprev], r[i], x[i]);    	// r -=  alpha^T * tmp[i]
		} else {
			#if USE_SPARSE
