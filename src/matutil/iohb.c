...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  ------------------------------
  fp_t PRECISION I/O FUNCTIONS
  ------------------------------
  FUNCTION:

  int readHB_newmat(const char *filename, int *M, int *N, *int nz,
  int **colptr, int **rowind,  fp_t**val)

  int readHB_mat(const char *filename, int *colptr, int *rowind,
  fp_t*val)
  ...
  format.  (See readHB_aux to read auxillary vectors.)
        -- Values are interpreted as fp_t precision numbers. --
  ...
  int readHB_aux(const char* filename, const char AuxType, fp_t b[])

  int readHB_newaux(const char* filename, const char AuxType, fp_t** b)
  ...
  int writeHB_mat(const char* filename, int M, int N, 
  ...
  The writeHB_mat function opens the named file and writes the specified
...
//			printf("%s %i\n", ThisElement, last);
          ...
          val[count] = strtod(ThisElement, NULL);
          count++; col += Valwidth;
       }
    }
    free(ThisElement);
...
int readHB_newmat(const char* filename, int* M, int* N, int* nonzeros, 
                         int** colptr, int** rowind, fp_t** val)
{
	int Nrhs;
        char *Type;
	...
	readHB_info(filename, &M, &N, &nonzeros, &Type, &Nrhs);
        if ( Nrhs <= 0 ) {
          ...
          if ( Type[0] == 'C' ) {
            ...
            *b = (fp_t *)malloc(M*Nrhs*sizeof(fp_t)*2);
            if ( *b == NULL ) IOHBTerminate("Insufficient memory for rhs.\n");
            return readHB_aux(filename, AuxType, *b);
          } else {
            *b = (fp_t *)malloc(M*Nrhs*sizeof(fp_t));
            if ( *b == NULL ) IOHBTerminate("Insufficient memory for rhs.\n");
	    return readHB_aux(filename, AuxType, *b);
          }
        }
...
/*  If available,  print right hand sides, 
    ...
    if ( fclose(out_file) != 0){
      fprintf(stderr,"Error closing file in writeHB_mat().\n");
      return 1;
    } else return 0;
