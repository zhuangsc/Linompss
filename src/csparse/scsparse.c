...
#define cs_add cs_add_single 
#define cs_cholsol cs_cholsol_single 
#define cs_cholsol2 cs_cholsol2_single 
#define cs_dupl cs_dupl_single 
#define cs_entry cs_entry_single 
#define cs_lusol cs_lusol_single 
#define cs_gaxpy cs_gaxpy_single 
#define cs_multiply cs_multiply_single 
#define cs_qrsol cs_qrsol_single 
#define cs_transpose cs_transpose_single 
#define cs_triplet cs_triplet_single 
#define cs_norm cs_norm_single 
#define cs_print cs_print_single 
#define cs_load cs_load_single 

#define cs_calloc cs_calloc_single 
#define cs_free cs_free_single 
#define cs_realloc cs_realloc_single 
#define cs_spalloc cs_spalloc_single 
#define cs_spfree cs_spfree_single 
#define cs_sprealloc cs_sprealloc_single 
#define cs_malloc cs_malloc_single 

#define cs_amd cs_amd_single 
#define cs_chol cs_chol_single 
#define cs_dmperm cs_dmperm_single 
#define cs_droptol cs_droptol_single 
#define cs_dropzeros cs_dropzeros_single 
#define cs_happly cs_happly_single 
#define cs_ipvec cs_ipvec_single 
#define cs_lsolve cs_lsolve_single 
#define cs_ltsolve cs_ltsolve_single 
#define cs_lu cs_lu_single 
#define cs_permute cs_permute_single 
#define cs_pinv cs_pinv_single 
#define cs_pvec cs_pvec_single 
#define cs_qr cs_qr_single 
#define cs_schol cs_schol_single 
#define cs_sqr cs_sqr_single 
#define cs_symperm cs_symperm_single 
#define cs_usolve cs_usolve_single 
#define cs_utsolve cs_utsolve_single 
#define cs_updown cs_updown_single 

#define cs_sfree cs_sfree_single 
#define cs_nfree cs_nfree_single 
#define cs_dfree cs_dfree_single 

#define cs_counts cs_counts_single 
#define cs_cumsum cs_cumsum_single 
#define cs_dfs cs_dfs_single 
#define cs_etree cs_etree_single 
#define cs_fkeep cs_fkeep_single 
#define cs_house cs_house_single 
#define cs_maxtrans cs_maxtrans_single 
#define cs_post cs_post_single 
#define cs_reach cs_reach_single 
#define cs_scc cs_scc_single 
#define cs_scatter cs_scatter_single 
#define cs_splsolve cs_splsolve_single 
#define cs_tdfs cs_tdfs_single 

#define cs_dalloc cs_dalloc_single 
#define cs_done cs_done_single 
#define cs_idone cs_idone_single 
#define cs_ndone cs_ndone_single 
#define cs_ddone cs_ddone_single 
...
*/
{
    ...
    x = values ? cs_malloc (m, sizeof (float)) : 0 ;
...
static int cs_diag (int i, int j, float aij, void *other) 
{ 
  return (i != j); 
}
...
*/
{
	...
	*hhead, *ATp, *ATi, d, dk, dext, lemax = 0, e, elenk, eln, i, j, k, k1,
	...
	C = A2 ? cs_multiply (AT, A2) : 0 ;	/* C=A'*A with no dense rows */
	cs_spfree (A2) ;
    }
    else
    {
...
int cs_cholsol (const cs *A, float *b, int order)
{
    float *x ;
    css *S ;
    csn *N ;
...
int cs_cholsol2 (int n, css *S, csn *N, float *b, float *x)
{
    ...
    float *tmp = cs_malloc (n, sizeof (float)) ;
    ok = (S && N && x) ;
    if (ok)
    {
...
static int cs_rprune (int i, int j, float aij, void *other)
{
    int *rr = (int *) other ;
...
static int cs_nonzero (int i, int j, float aij, void *other)
{
    return (aij != 0) ;
}
...
int cs_happly (const cs *V, int i, float beta, float *x)
{
    int p, *Vp, *Vi ;
    float *Vx, tau = 0 ;
...
int cs_ipvec (int n, const int *P, const float *b, float *x)
{
    int k ;
...
int cs_lsolve ( const cs *L, float *x )
/*
  Purpose:
...
int cs_ltsolve ( const cs *L, float *x )
/*
  Purpose:
...
int cs_lusol (const cs *A, float *b, int order, float tol)
{
    float *x ;
    css *S ;
    csn *N ;
...
int cs_pvec (int n, const int *P, const float *b, float *x)
{
    int k ;
    if (!x || !b) return (0) ;				    /* check inputs */
...
csn *cs_qr (const cs *A, const css *S)
{
	...
	*Ai, *parent, *Rp, *Ri, *Vp, *Vi, *w, *Pinv, *Q ;
    ...
    N->B = Beta = cs_malloc (n, sizeof (float)) ;
...
int cs_qrsol (const cs *A, float *b, int order)
{
    float *x ;
    css *S ;
    csn *N ;
	...
	x = cs_calloc (S ? S->m2 : 1, sizeof (float)) ;
	ok = (S && N && x) ;
	if (ok)
	{
...
int cs_splsolve (cs *L, const cs *B, int k, int *xi, float *x, const int *Pinv)
{
...
int cs_updown (cs *L, int sigma, const cs *C, const int *parent)
{
    ...
    float *Lx, *Cx, alpha, beta = 1, delta, gamma, w1, w2, *w, n,  beta2 = 1 ;
    if (!L || !C || !parent) return (0) ;
    Lp = L->p ; Li = L->i ; Lx = L->x ; n = L->n ;
    Cp = C->p ; Ci = C->i ; Cx = C->x ;
    if ((p = Cp [0]) >= Cp [1]) return (1) ;	    /* return if C empty */
    w = cs_malloc (n, sizeof (float)) ;
    if (!w) return (0) ;
    f = Ci [p] ;
...
int cs_usolve (const cs *U, float *x)
{
    int p, j, n, *Up, *Ui ;
    float *Ux ;
...
cs *cs_spalloc (int m, int n, int nzmax, int values, int triplet)
{
    ...
    A->x = values ? cs_malloc (nzmax, sizeof (float)) : 0 ;
...
int cs_utsolve (const cs *U, float *x)
{
    int p, j, n, *Up, *Ui ;
    float *Ux ;
    if (!U || !x) return (0) ;				    /* check inputs */
