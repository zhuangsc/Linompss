...
int matmul_check(int check, int b, int d, int c, int m, int n, int k, int transa, int transb, void *A, void *B, int ldb, void *C, int ldc) 
{
	if ( check ) {
		...
		m = Ahb->m; /* have not been set in the main routine, if using a sparse A */
		k = Ahb->n;
		ldb = k;
		ldc = m;
		...
		SBLAS_csrmm("N", m, n, k, FP_ONE, "GLNF", Ahb->vval, Ahb->vpos, Ahb->vptr, Ahb->vptr+1, Bcm, ldb, FP_NOUGHT, Cx, ldc);
