...
if (length(argv) != 3)
	printf("./bjacobi.m [Matrix A] [Vector B] [Block size]\n");
	quit
endif
...
for iter = 1 : MAX_ITER
	...
	for i = 1 : BS : DIM
		for j = 1 : BS : DIM
			if ( i != j )
				Xtmp(i:i+BS-1,:) += MT(i:i+BS-1,j:j+BS-1) * X0(j:j+BS-1,:);
			endif
		endfor
		Xtmp(i:i+BS-1,:) = B(i:i+BS-1,:) - Xtmp(i:i+BS-1,:);
		LL = chol(MT(i:i+BS-1,i:i+BS-1),"lower");
		LU = chol(MT(i:i+BS-1,i:i+BS-1),"upper");
		X0(i:i+BS-1,:) = linsolve(LL, Xtmp(i:i+BS-1,:));
